...
export default class Sprites {
	...
	get(group : string, entity : string, cycle : string, frame : number) : Object {
		...
		if(!this.index) {
			...
			for(let g in this.info.group) {
				...
				for(let i in this.info.group[g].entity) {
					...
					this.index[g][e] = {};
					for(let j in this.info.group[g].cycle) {
						let c = this.info.group[g].cycle[j];
						this.index[g][e][c] = [];
						for(let f=0; f < this.info.frames_per_cycle; f++) {
							this.index[g][e][c][f] = {
								x: this.size.x*(this.info.frames_per_cycle * (j|0) + f),
		...
		return {img: this.img, rect: this.index[group][entity][cycle][frame]};
	}
}
